...
load_relocate_program:                      ;加载并重定位用户程序
  ...
  .b1:
         ...
         mov [esi+0x14],ecx                 ;登记TSS的线性基地址到TCB

         ;以下申请创建LDT所需要的内存
         mov ebx,esi
         mov ecx,160                        ;允许安装20个LDT描述符
         call flat_4gb_code_seg_sel:task_alloc_memory
         mov [esi+0x0c],ecx                 ;登记LDT的线性基地址到TCB中
         ...
         mov edx,[esi+0x06]                 ;堆栈的高端线性地址=下一可分配地址
         mov [ebx+56],edx                   ;填写TSS的ESP域

         ;重定位SALT 
         cld

         mov ecx,[0x0c]                     ;U-SALT条目数(通过访问4GB段取得)
         mov edi,[0x08]                     ;U-SALT在4GB段内的偏移
  .b2: 
         push ecx
         push edi
...
start:
  ...
  .g0:
         ...
         mov eax,cr3
         mov dword [ecx+28],eax             ;登记CR3(PDBR)
